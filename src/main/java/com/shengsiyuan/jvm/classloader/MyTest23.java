 ...
 * 所共同决定的。如果同样名字 (即相同的完全限定名) 的类是由两个不同的加载器所加载, 那么这些类就是不同的, 即使.class
 * 文件的字节码完全一样, 并且从相同的位置加载亦如此。(数组不是由类加载器加载, 而是在运行期间由VM虚拟机来创建)
 * <p>
 * 在Oracle的Hotspot实现中, 系统属性sun.boot.classes.path如果修改错了, 则运行出错, 提示如下错误信息:
...
public class MyTest23 {
    public static void main(String[] args) {
        ...
        /*
            ...
            当JVM启动时, 一块特殊的机器代码会运行, 它会加载扩展类加载器与系统类加载器,
            这块特殊的机器码叫做启动类加载器 (Bootstrap) 。

            启动类加载器并不是Java类, 而其他的加载器则是Java类,
            启动类加载器时特定于平台的机器指令, 它负责开启整个加载过程。
